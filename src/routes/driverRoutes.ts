...
export function validateRouteLocations(
  method: string,
  body:
    | Pick<UpdateRoutePayload, 'origin' | 'destination'>
    | undefined
): string | null {
...
function rejectUnresolvedCoordinates(req: Request, res: Response, next: NextFunction) {
...
router.post('/routes', rejectUnresolvedCoordinates, asyncHandler(async (
...
router.put('/routes/:id', rejectUnresolvedCoordinates, asyncHandler(async (
