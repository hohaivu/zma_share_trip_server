...
describe('MVC wallet-gated driver route publishing', () => {
  it('publishing a draft reserves the route fee and appends a ledger entry', async () => {
    ...
    const route = await driverRouteService.createRoute(DRIVER_001_ID, {
      ...
      departureDate: '2030-04-30T07:00:00.000Z',
      windowStart: '2030-04-30T07:00:00.000Z',
      windowEnd: '2030-04-30T07:00:00.000Z',
      tripPrice: 100000,
  ...
  it('rejects publish when the wallet balance is insufficient and keeps the route in draft', async () => {
    ...
    const route = await driverRouteService.createRoute(DRIVER_001_ID, {
      ...
      origin: { lat: 10.77, lng: 106.7, label: 'Q1' },
      destination: { lat: 10.85, lng: 106.75, label: 'TD' },
      departureDate: '2030-05-01T07:00:00.000Z',
      windowStart: '2030-05-01T07:00:00.000Z',
      windowEnd: '2030-05-01T07:00:00.000Z',
      tripPrice: 120000,
      distanceMeters: 10000,
    })
  ...
  it('rejects in-place edits to fee-bearing fields after publish', async () => {
    ...
    const route = await driverRouteService.createRoute(DRIVER_001_ID, {
      ...
      origin: { lat: 10.77, lng: 106.7, label: 'Q1' },
      destination: { lat: 10.85, lng: 106.75, label: 'TD' },
      departureDate: '2030-05-02T07:00:00.000Z',
      windowStart: '2030-05-02T07:00:00.000Z',
      windowEnd: '2030-05-02T07:00:00.000Z',
      tripPrice: 130000,
...
describe('MVC wallet-gated accept and cancel transitions', () => {
    ...
    const route = await driverRouteService.publishRoute(
      (
        await driverRouteService.createRoute(DRIVER_001_ID, {
          ...
          origin: { lat: 10.77, lng: 106.7, label: 'Q1' },
          destination: { lat: 10.85, lng: 106.75, label: 'TD' },
          departureDate: '2030-06-03T07:00:00.000Z',
          windowStart: '2030-06-03T07:00:00.000Z',
          windowEnd: '2030-06-03T07:00:00.000Z',
          tripPrice: 130000,
          distanceMeters: 10000,
        })
      ).id,
    ...
    const declinedRoute = await driverRouteService.publishRoute(
      (
        await driverRouteService.createRoute(DRIVER_001_ID, {
          ...
          origin: { lat: 10.77, lng: 106.7, label: 'Q1' },
          destination: { lat: 10.85, lng: 106.75, label: 'TD' },
          departureDate: '2030-06-04T07:00:00.000Z',
          windowStart: '2030-06-04T07:00:00.000Z',
          windowEnd: '2030-06-04T07:00:00.000Z',
          tripPrice: 140000,
          distanceMeters: 10000,
        })
      ).id,
    ...
    const canceledRoute = await driverRouteService.publishRoute(
      (
        await driverRouteService.createRoute(DRIVER_001_ID, {
          ...
          origin: { lat: 10.77, lng: 106.7, label: 'Q1' },
          destination: { lat: 10.85, lng: 106.75, label: 'TD' },
          departureDate: '2030-06-05T07:00:00.000Z',
          windowStart: '2030-06-05T07:00:00.000Z',
          windowEnd: '2030-06-05T07:00:00.000Z',
          tripPrice: 150000,
          distanceMeters: 10000,
        })
      ).id,
    ...
    const route = await driverRouteService.publishRoute(
      (
        await driverRouteService.createRoute(DRIVER_001_ID, {
          ...
          origin: { lat: 10.77, lng: 106.7, label: 'Q1' },
          destination: { lat: 10.85, lng: 106.75, label: 'TD' },
          departureDate: '2030-05-03T07:00:00.000Z',
          windowStart: '2030-05-03T07:00:00.000Z',
          windowEnd: '2030-05-03T07:00:00.000Z',
          tripPrice: 140000,
          distanceMeters: 10000,
        })
      ).id,
    ...
    const route = await driverRouteService.publishRoute(
      (
        await driverRouteService.createRoute(DRIVER_001_ID, {
          ...
          origin: { lat: 10.77, lng: 106.7, label: 'Q1' },
          destination: { lat: 10.85, lng: 106.75, label: 'TD' },
          departureDate: '2030-05-04T07:00:00.000Z',
          windowStart: '2030-05-04T07:00:00.000Z',
          windowEnd: '2030-05-04T07:00:00.000Z',
          tripPrice: 150000,
          distanceMeters: 10000,
        })
      ).id,
    ...
    const route = await driverRouteService.publishRoute(
      (
        await driverRouteService.createRoute(DRIVER_001_ID, {
          ...
          origin: { lat: 10.77, lng: 106.7, label: 'Q1' },
          destination: { lat: 10.85, lng: 106.75, label: 'TD' },
          departureDate: '2030-05-05T07:00:00.000Z',
          windowStart: '2030-05-05T07:00:00.000Z',
          windowEnd: '2030-05-05T07:00:00.000Z',
          tripPrice: 160000,
          distanceMeters: 10000,
        })
      ).id,
  ...
  it('refunds a charged fee and cancels the winning plan-side acceptance', async () => {
    ...
    const plan = await planService.createPlan(CLIENT_001_ID, {
      ...
      departureDate: '2030-05-06T07:00:00.000Z',
    ...
    const route = await driverRouteService.publishRoute(
      (
        await driverRouteService.createRoute(DRIVER_001_ID, {
          ...
          departureDate: '2030-05-06T07:00:00.000Z',
          windowStart: '2030-05-06T07:00:00.000Z',
          windowEnd: '2030-05-06T07:00:00.000Z',
          tripPrice: 170000,
          distanceMeters: 10000,
        })
      ).id,
